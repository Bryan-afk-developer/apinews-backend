...
const get = async (request, response) => {
    try {
        ...
        const entities = await New.findAll({ where: filters, include: relations });
...
const getById = async (request, response) => {
    try {
        ...
        const entitie = await New.findByPk(id, { include: relations });
        if (entitie) {
...
const create = async (request, response) => {
    try {
        ...
        // Limpiamos el campo imagen si viene como Data URI
        if (data.imagen && data.imagen.startsWith('data:')) {
            data.imagen = data.imagen.split(',')[1];
        }

        data.UserAlta = "Admin"; // O request.user.id
        data.FechaAlta = new Date();

        const newEntitie = await New.create(data);
        response.status(201).json(newEntitie);
    } catch (err) {
        console.error("ERROR DETALLADO AL CREAR NOTICIA:", err);
        response.status(500).send('Error al crear la noticia. Revisa la consola.');
    }
};
